...
namespace myProducts.WarmupHelper
{
    class Program
    {
        static void Main()
        {
            var localClient = new HttpClient { BaseAddress = new Uri("https://windows8vm/ngmd/api/") };
            localClient.SetBasicAuthentication("cw", "cw");

            var cloudClient = new HttpClient { BaseAddress = new Uri("https://demo.christianweyer.net/api/") };
